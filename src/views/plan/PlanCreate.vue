            ...
            <div class="col-md-12">
               <form class="my-3 fill-row-form" onsubmit="return false;" role="search">
                  <select id="search-area" class="form-select me-2"
                     v-model="sido" @change="showValue(sido)">
                     <option value="0" selected>검색 할 지역 선택</option>
                     <option v-for="sido in sidos" :key="sido.sidoCode" :value="sido.sidoCode">{{ sido.sidoName }}</option>
                  </select>
                  <select id="search-town" class="form-select me-2" 
                  v-model="gugun">
                     <option value="0" selected>구군 선택</option>
                     <option v-for="gugun in filteredGuguns" :key="gugun.gugunCode" :value="gugun.gugunCode">{{ gugun.gugunName }}</option>
                  </select>
                           ...
                           <table id="top-table" class="w-100" style="display: none">
                              <thead>
                                 <tr>
                                    ...
                                    <th>관광지명</th>
                                    <th>주소</th>
                                    <th>삭제</th> <!-- New column for delete button -->
                                 </tr>
                              </thead>
                              <tbody id="trip-select">
                                 <tr v-for="(attraction, index) in attractionList" :key="attraction.contentId" @click="moveCenter(attraction.latitude, attraction.longitude)">
                                    <td>{{ index + 1 }}</td>
                                    <td><img :src="attraction.firstImage" width="100px"></td>
                                    <td>{{ attraction.title }}</td>
                                    <td>{{ attraction.addr1 }} {{ attraction.addr2 }}</td>
...
methods: {
   ...
   getRecommend(prevAttraction) {
      if(prevAttraction === null) {
         axios.get("http://localhost/attraction/rest/12/1/1")
         .then((response) => {
   ...
   getSido() {
      axios.get("http://localhost/attraction/rest")
      .then((response) => {
